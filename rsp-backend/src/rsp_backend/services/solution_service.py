...
async def list_solutions(db: Connection, search: str | None = None) -> list[Solution]:
    if search:
        rows = await db.fetch(
            "SELECT * FROM solutions WHERE title ILIKE $1 OR description ILIKE $1 OR code ILIKE $1 ORDER BY created_at DESC",
            f"%{search}%",
        )
    else:
...
async def create_solution(db: Connection, data: SolutionCreate) -> Solution:
    row = await db.fetchrow(
        """INSERT INTO solutions
           (code, title, description, test_method, equipment,
            cost_impact, weight_impact, complexity_level, verified)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *""",
        data.code, data.title, data.description, data.test_method,
        json.dumps(data.equipment), data.cost_impact, data.weight_impact,
        data.complexity_level, data.verified,
    )
...
async def update_solution(db: Connection, solution_id: str, data: SolutionUpdate) -> Solution | None:
    ...
    cost_impact = data.cost_impact if data.cost_impact is not None else existing.cost_impact
    weight_impact = data.weight_impact if data.weight_impact is not None else existing.weight_impact
    complexity_level = data.complexity_level if data.complexity_level is not None else existing.complexity_level
    verified = data.verified if data.verified is not None else existing.verified

    row = await db.fetchrow(
        """UPDATE solutions SET code=$1, title=$2, description=$3, test_method=$4,
           equipment=$5, cost_impact=$6, weight_impact=$7, complexity_level=$8,
           verified=$9, updated_at=NOW() WHERE id=$10 RETURNING *""",
        code, title, description, test_method, equipment, cost_impact,
        weight_impact, complexity_level, verified, solution_id,
    )
    return Solution.model_validate(dict(row))
...
async def create_solution_step(db: Connection, solution_id: str, data: SolutionStepCreate) -> SolutionStep:
    row = await db.fetchrow(
        """INSERT INTO solution_steps
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *""",
        solution_id, data.order_index, data.title, data.description,
        json.dumps(data.input_params_template), data.duration_estimate_minutes,
...
async def link_solution_risk(
    ...
    try:
        await db.execute(
            """INSERT INTO solution_risks (solution_id, risk_id, recommendation_level)
               VALUES ($1, $2, $3)""",
            solution_id, risk_id, recommendation_level,
        )
        return True
    except Exception:
        return False
