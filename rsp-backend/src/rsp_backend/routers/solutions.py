...
from rsp_backend.services import solution_service
...
async def create_solution_step(
    solution_id: str, body: SolutionStepCreate, db: Connection = Depends(get_connection),
):
...
async def update_solution_step(
    solution_id: str, step_id: str, body: SolutionStepUpdate, db: Connection = Depends(get_connection),
):
...
async def delete_solution_step(
    solution_id: str, step_id: str, db: Connection = Depends(get_connection),
):
...
async def reorder_solution_steps(
    solution_id: str, body: ReorderRequest, db: Connection = Depends(get_connection),
):
...
async def link_solution_risk(
    solution_id: str, body: LinkRiskRequest, db: Connection = Depends(get_connection),
):
    ok = await solution_service.link_solution_risk(
        db, solution_id, body.risk_id, body.recommendation_level,
    )
    if not ok:
...
async def unlink_solution_risk(
    solution_id: str, risk_id: str, db: Connection = Depends(get_connection),
):
