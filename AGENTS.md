...
- Registered class families (`tpl-frontend/src/lib/`): `fieldtypes.ts` (definition kinds number/text/select/bool/struct — each with `paramsSchema` + `describe(params)`), `values.ts` (binding value classes: plain/ramp/tolerance/percentage/sinusoidal/struct — expose `values()` = characteristic set with per-channel `transformable` flag, plus `describe()`/`display()` custom summaries, `scalar()`), `structs.ts` (`GearboxStruct{ratio}`, `ProductStruct{ratio, model, nominal_output_speed, nominal_input_torque, efficiency}`, `BenchStruct{unit_a_side}`), `transforms.ts` (typed methods: formula/linear/lookup/`gearbox.trans`/`product.trans`/`product.back2back`/`b2b.route`; static `inputs()` + `outputs()` + `apply(inputs, params) → Record<role, Value>`). Every transform declares one or more output ports; each output binds to a derived def whose name/unit are read from that def. `product.trans` params: `ratio_mode` (none/multiply/divide), `reference` (none/speed/torque — which product nominal a percentage resolves against), `efficiency` (none/multiply/divide); combined as a factor, percentage resolved via nominal unless reference=none. `product.back2back`: inputs `speed`+`torque` (commanded primary quantities) +`unit_a`+`unit_b` (struct products) +`run_on` (select whose option encodes primary unit + mode, e.g. `A:motor`/`A:generator`/`B:motor`/`B:generator`, `:`/`-`, case-insensitive; empty/unparsable ⇒ no output), outputs `speed`+`torque` (each factor per reference×runmode; unbound input ⇒ omitted output); LSS-LSS coupled. `b2b.route`: inputs `speed`+`torque` (b2b outputs) +`run_on` +`bench` (struct `bench`), outputs `left_speed`/`left_torque`/`right_speed`/`right_torque`; routes speed/torque to the left/right motor system per `bench.unit_a_side` (Unit B on the other side) and mode (motor: speed→primary side, torque→companion; generator: swapped).
...
- **Transform outputs are composable**: `Transform.apply` returns a `Record<role, Value>`; each `Value` lazily recomputes from its input `Value`s, so chaining works (e.g. gearbox-converted features → linear ×2) and one transform can emit multiple outputs (e.g. `product.back2back` speed + torque).
- **Time interpretation (Signals page)**: `lib/signals.ts` optionally evaluates characteristic sets over each step's active window for the read-only `/documents/:id/signals` view (no changes to stored documents). Shape is detected from channel names, so derived outputs work too: `amplitude` ⇒ sinusoidal, `start_value`/`end_value` ⇒ ramp, `value`+`reference` ⇒ percentage, otherwise constant (tolerance adds a shaded band). Ramp uses `ramp_rate` clamped to the start/end range when set, else linear across `duration_minutes`; sinusoidal is `offset + amplitude·sin(2π·frequency·t)` reset per execution; percentage is `value·reference/100`. Timeline is serial `(duration + changeover) × required_executions`, x = elapsed from 0, one lane per selected numeric/derived signal, changeover windows are gaps. Selection persists in `localStorage["tpl.signals.selected.<id>"]`; the chart supports horizontal zoom (buttons or Ctrl/⌘+wheel) with zoom-aware tick density.
- Persisted shapes (`tpl-frontend/src/types/plan.ts`):
  - `PlanDocument`: `{ version, definitions{input_conditions, collection_items, completion_criteria, custom}, root, templates, transforms, input_layout }`. `input_layout` is an optional global layout for Input Conditions: `[{definition_id, size?, hidden?}]` where array order = display order, `size` is `sm|md|lg` (25/50/100% width on the logging page, default `md`), and `hidden` omits the input from the logging page (kept last in the layout editor); empty/missing means definition order with default size. The logging page also shows derived/computed inputs when they compute for the selected step; hidden derived inputs are display-only (their values are still saved in run readings).
